...
export const REQUIRED_CONSTRAINTS: readonly ConstraintContract[] = [
  ...
  checkConstraint("one_time_tokens_purpose_check", "CHECK (purpose = ANY (ARRAY['signup'::text, 'email_change'::text, 'recovery'::text, 'magic_link'::text, 'email_otp'::text, 'invite'::text]))"),
  ...
  checkConstraint("oauth_states_flow_check", "CHECK (btrim(flow) <> ''::text)"),
